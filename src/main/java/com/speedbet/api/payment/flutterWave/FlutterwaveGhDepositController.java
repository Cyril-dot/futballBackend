 ...
 *        POST /api/webhooks/flutterwave/gh
 ...
 * this and the Nigeria controller are both deployed, only one of these
...
public class FlutterwaveGhDepositController extends AbstractFlutterwaveDepositController {
    ...
    private static final String CHARGE_TYPE       = "mobile_money_ghana";
    private static final String EXPECTED_CURRENCY = "GHS";
    private static final String PROVIDER_TAG      = "flutterwave_gh";
    ...
    public ResponseEntity<ApiResponse<Map<String, Object>>> initDeposit(
        ...
        body.put("redirect_url", frontendUrlOverride + "/app/wallet?payment=success");
    ...
    public ResponseEntity<String> webhook(
        ...
        try {
            ...
            log.error("Flutterwave GH webhook: failed to read request body", e);
            return ResponseEntity.status(400).body("Failed to read body");
        }

        if (!verifyWebhookHash(verifHash)) {
            log.warn("Flutterwave GH webhook: invalid or missing verif-hash");
        ...
        try {
            ...
            log.info("Flutterwave GH webhook: received event='{}'", eventType);

            if (!"charge.completed".equals(eventType)) {
                log.info("Flutterwave GH webhook: ignoring event='{}'", eventType);
            ...
            if (webhookData == null || webhookData.get("id") == null) {
                log.error("Flutterwave GH webhook: missing data.id in payload");
            ...
            if (!"successful".equals(status)) {
                log.info("Flutterwave GH webhook: flwTransactionId={} verified status='{}' — not crediting",
                        flwTransactionId, status);
                return ResponseEntity.ok("Not successful, ignored");
            }

            if (!EXPECTED_CURRENCY.equals(currency)) {
                log.error("Flutterwave GH webhook: flwTransactionId={} unexpected currency='{}' (expected {})",
            ...
            if (meta == null || meta.get("userId") == null) {
                log.error("Flutterwave GH webhook: verified data missing meta.userId, flwTransactionId={}",
                        flwTransactionId);
            ...
            log.error("Flutterwave GH webhook: bad request — {}", e.getMessage(), e);
            return ResponseEntity.status(400).body("Bad request: " + e.getMessage());
        } catch (Exception e) {
            log.error("Flutterwave GH webhook: unexpected error — will retry", e);
